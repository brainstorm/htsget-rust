...
- API version: 1.1.1
- Build date: 2019-02-25T20:32:05.144279+11:00[Australia/Melbourne]
...
```
cargo run --example client SearchReadId
cargo run --example client SearchVariantId
```
...
* Add `openapi_client = {version = "1.1.1", path = "openapi_client"}` under `[dependencies]` in the root `Cargo.toml`.
