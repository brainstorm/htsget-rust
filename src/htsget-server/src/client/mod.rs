...
impl<F, C> Api<C> for Client<F> where
    ...
    fn search_read_id(&self, param_id: String, param_format: Option<String>, param_reference_name: Option<String>, param_start: Option<i64>, param_end: Option<i64>, param_fields: Option<String>, param_tags: Option<String>, param_notags: Option<String>, context: &C) -> Box<Future<Item=SearchReadIdResponse, Error=ApiError>> {

        // Query parameters
        let query_format = param_format.map_or_else(String::new, |query| format!("format={format}&", format=query.to_string()));
        let query_reference_name = param_reference_name.map_or_else(String::new, |query| format!("referenceName={reference_name}&", reference_name=query.to_string()));
        let query_start = param_start.map_or_else(String::new, |query| format!("start={start}&", start=query.to_string()));
        let query_end = param_end.map_or_else(String::new, |query| format!("end={end}&", end=query.to_string()));
        let query_fields = param_fields.map_or_else(String::new, |query| format!("fields={fields}&", fields=query.to_string()));
        let query_tags = param_tags.map_or_else(String::new, |query| format!("tags={tags}&", tags=query.to_string()));
        let query_notags = param_notags.map_or_else(String::new, |query| format!("notags={notags}&", notags=query.to_string()));


        let uri = format!(
            "{}/brainkod/htsget/1.1.1/reads/{id}?{format}{reference_name}{start}{end}{fields}{tags}{notags}",
            self.base_path, id=utf8_percent_encode(&param_id.to_string(), ID_ENCODE_SET),
            format=utf8_percent_encode(&query_format, QUERY_ENCODE_SET),
            reference_name=utf8_percent_encode(&query_reference_name, QUERY_ENCODE_SET),
            start=utf8_percent_encode(&query_start, QUERY_ENCODE_SET),
            end=utf8_percent_encode(&query_end, QUERY_ENCODE_SET),
            fields=utf8_percent_encode(&query_fields, QUERY_ENCODE_SET),
            tags=utf8_percent_encode(&query_tags, QUERY_ENCODE_SET),
            notags=utf8_percent_encode(&query_notags, QUERY_ENCODE_SET)
        );
                    ...
                    Box::new(
                        ...
                        .and_then(|body| str::from_utf8(&body)
                                             ...
                                             .and_then(|body|
                                                 ...
                                                 serde_json::from_str::<models::HtsgetResponse>(body)
                                                     .map_err(|e| e.into())

                                             ))
                        .map(move |body|
                            SearchReadIdResponse::ResultsMatchingCriteria(body)
                        )
                    ...
                    Box::new(
                        ...
                        .and_then(|body| str::from_utf8(&body)
                                             ...
                                             .and_then(|body|
                                                 ...
                                                 serde_json::from_str::<models::ErrorResponses>(body)
                                                     .map_err(|e| e.into())

                                             ))
                        .map(move |body|
                            SearchReadIdResponse::SomethingWentWrong(body)
                        )
                    ) as Box<Future<Item=_, Error=_>>
                },
                500 => {
    ...
    fn search_variant_id(&self, param_id: String, param_format: Option<String>, param_reference_name: Option<String>, param_start: Option<i64>, param_end: Option<i64>, param_fields: Option<String>, param_tags: Option<String>, param_notags: Option<String>, context: &C) -> Box<Future<Item=SearchVariantIdResponse, Error=ApiError>> {
        ...
        let uri = format!(
            "{}/brainkod/htsget/1.1.1/variants/{id}?{format}{reference_name}{start}{end}{fields}{tags}{notags}",
            self.base_path, id=utf8_percent_encode(&param_id.to_string(), ID_ENCODE_SET),
            format=utf8_percent_encode(&query_format, QUERY_ENCODE_SET),
            reference_name=utf8_percent_encode(&query_reference_name, QUERY_ENCODE_SET),
            start=utf8_percent_encode(&query_start, QUERY_ENCODE_SET),
            ...
            match response.status().as_u16() {
                200 => {
                    ...
                    Box::new(
                        ...
                        .map(move |body|
                            SearchVariantIdResponse::ResultsMatchingCriteria(body)
                        )
                    ) as Box<Future<Item=_, Error=_>>
                },
                400 => {
                    ...
                    Box::new(
                        ...
                        .map(move |body|
                            SearchVariantIdResponse::SomethingWentWrong(body)
                        )
                    ) as Box<Future<Item=_, Error=_>>
                },
                500 => {
